...
| `server.bat` / `server.ps1` | Windows 用起動スクリプト（Docker Compose 起動 + ブラウザ自動オープン）。.bat はダブルクリック用ランチャー |
| `stop.bat` / `stop.ps1` | Windows 用停止スクリプト（Docker Compose 停止）。.bat はダブルクリック用ランチャー |
| `デスクトップに追加.bat` / `デスクトップに追加.ps1` | デスクトップにショートカットを作成する Windows 用セットアップスクリプト |
| `デスクトップから削除.bat` / `デスクトップから削除.ps1` | デスクトップのショートカットを削除する Windows 用クリーンアップスクリプト |
